import { CHON_GHE, CHON_NGAY_CHIEU } from "../Actions/DatVePhimTypes";
...
const DatVePhimReducer = (state = stateDefaut, action) => {
  switch (action.type) {
    case CHON_GHE: {
      let { mangGheChon, mangGhe } = state;
      let { gheChon } = action;
      //
      let indexGheChon = mangGheChon.findIndex(
        (item) => item.soGhe === gheChon.soGhe
      );

      //Kiểm tra ghế chọn đã bán hay chưa, kết quả: 
      //Đã bán: -1, chưa bán: 0
      let isSell = mangGhe.findIndex((item) => {
        let ghe = item.danhSachGhe.find(
          (item) => item.soGhe === gheChon.soGhe && gheChon.daDat === true
        );
        if (ghe) {
          return true;
        }
      ...
      if (isSell === -1) {
        if (indexGheChon === -1) {
          mangGheChon.push(gheChon);
        } else {
          mangGheChon.splice(indexGheChon, 1);
        }
      }
      state.mangGheChon = mangGheChon;
      return { ...state };
    }
    case CHON_NGAY_CHIEU: {
      state.ngayChieu = action.ngayChieu;
      return { ...state };
    }
    default:
      return { ...state };
